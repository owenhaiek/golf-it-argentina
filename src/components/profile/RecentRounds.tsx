...
const RecentRounds = ({
  ...
  const deleteRoundMutation = useMutation({
    mutationFn: async (roundId: string) => {
      ...
      const {
        error
      } = await supabase.from('rounds').delete().eq('id', roundId).eq('user_id', userId);
      if (error) {
  ...
  return <Card className="border-0 shadow-md overflow-hidden h-full">
      ...
      <CardContent className="pt-4 py-[6px]">
        {rounds && rounds.length > 0 ? <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
            {rounds.map(round => {
          const isDeleting = deletingRoundId === round.id;
          const formattedDate = format(new Date(round.date || round.created_at), 'MMM d, yyyy');
          return <div key={round.id} className="group relative rounded-xl overflow-hidden bg-white shadow-lg hover:shadow-xl transition-all duration-200 border border-muted/10 flex flex-col">
                  {/* Image Section */}
                  <div className="relative">
                    {round.golf_courses.image_url ? <div className="w-full h-32 overflow-hidden">
                      ...
                      </div> : <div className="w-full h-32 bg-gradient-to-r from-secondary/30 to-primary/20 flex items-center justify-center">
                  ...
                  <div className="p-4 flex-grow flex flex-col">
                    <div>
                      ...
                      {round.golf_courses.city && <p className="text-sm text-muted-foreground flex items-center gap-1 mb-2">
                    ...
                    <Button variant="ghost" size="sm" className="mt-3 text-red-500 hover:bg-red-50 hover:text-red-600 transition-colors w-full" onClick={() => handleDeleteRound(round.id)} disabled={isDeleting}>
